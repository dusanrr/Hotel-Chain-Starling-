...
<%
    ...
    String sqlPagination="select SQL_CALC_FOUND_ROWS * from `rezervacije` R inner join `tipovisoba` RT inner join `hoteli` H inner join `klijenti` C where C.ID=R.klijentID and H.ID=R.hotelID and RT.ID=R.tipsobeID and R.hotelID="+user.getHotelID()+" limit "+iPageNo+","+iShowRows+"";
...
%>                     
        ...
        {
         %>
          <a class='page gradient' href="${pageContext.request.contextPath}/menadzer/rezervacije.jsp?iPageNo=<%=prePageNo%>&cPageNo=<%=prePageNo%>"> << Previous</a>
         <%
        }
        ...
        for(i=((cPage*iTotalSearchRecords)-(iTotalSearchRecords-1));i<=(cPage*iTotalSearchRecords);i++)
        {
          ...
          {
          %>
           <a class='page gradient' href="${pageContext.request.contextPath}/menadzer/rezervacije.jsp?iPageNo=<%=i%>" style="color: #59d"><b><%=i%></b></a>
          <%
          }
          else if(i<=iTotalPages)
          {
          %>
           <a class='page gradient' href="${pageContext.request.contextPath}/menadzer/rezervacije.jsp?iPageNo=<%=i%>"><%=i%></a>
          <% 
          }
        }
        ...
        {
         %>
         <a class='page active' href="${pageContext.request.contextPath}/menadzer/rezervacije.jsp?iPageNo=<%=i%>&cPageNo=<%=i%>"> >> Next</a> 
         <%
        }
        }
      %>
